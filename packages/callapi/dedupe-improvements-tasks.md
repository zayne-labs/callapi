...
export const createDedupeStrategy = async (context: DedupeContext) => {
 ...
 if (resolvedDedupeStrategy === "throttle") {
  ...
  if (!throttle.canExecute(dedupeKey)) {
   throw new Error(`Request throttled: exceeded ${throttleConfig.maxRequests} requests per ${throttleConfig.timeWindow}ms`);
  }
 }
  ...
  return {
   handleRequestDeferStrategy: async (deferContext) => {
    return debounce.schedule(dedupeKey, async () => {
     ...
     const streamableRequest = await toStreamableRequest({ ...context, options: localOptions, request: localRequest });

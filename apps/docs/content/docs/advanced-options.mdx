...
Use a plain object when you want actual default metadata. Its shape does not register a type for the `callApi` instance. Individual calls take metadata values, not `metaHelper()`.

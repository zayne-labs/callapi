...
CallApi can automatically retry failed requests. Retries are off by default (`retryAttempts: 0`). For most use cases, you only need to specify the number of retry attempts:
...
2. **Retry Status Codes**: Specifies which HTTP status codes should be retried. Defaults to `[]`, which makes every error status code eligible for retry. Network errors have no status code, so they are always eligible, whatever this list contains.
...
In these cases, you can use the `refetch()` function available in most error-related hooks. See the [Manual Refetching](/docs/hooks#manual-refetching) section in the Hooks documentation for more details.

...
- `query` - Validates query string parameters before adding to the URL
...
// @errors: 2741
const { data } = await callApi("/analytics", {
	meta: {},
});
```
...
1. Using colon syntax in the schema path (`:paramName`) - Enforces types at the type level only
2. Using the `params` validator schema - Validates at both type level and runtime (takes precedence over colon syntax)
...
2. Prefixing the path with `@method-name` (supported: `@get/`, `@post/`, `@put/`, `@patch/`, `@delete/`)
...
const waitUntil = (milliseconds: number) => new Promise((resolve) => setTimeout(resolve, milliseconds));

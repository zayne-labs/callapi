...
You can define a fallback schema that applies to all routes not explicitly defined in your schema using the special `.` key (or use the exported `fallBackRouteSchemaKey` constant):

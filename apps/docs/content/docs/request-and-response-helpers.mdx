...
CallApi provides the `toFormData` utility that converts objects to FormData.

```ts title="api.ts"
import { callApi } from "@zayne-labs/callapi";
import { toFormData } from "@zayne-labs/callapi/utils";

const result = await callApi("https://api.example.com/upload", {
	method: "POST",
	body: toFormData({ name: "John", age: 30, active: true }),
});
```

The utility handles various data types:

- **Primitives/Files/Blobs**: Added directly to FormData
- **Arrays**: Each item is appended to the same key (allows multiple values)
...
**ReturnType-preserving mode**

When using schema validation libraries like Zod, you can preserve the input type while converting to FormData at runtime. This is useful when your callApi schema expects the original object type but needs to be transformed to FormData for transmission:
...
const validatedData = uploadSchema.parse({ name: "John", file: blob });

const result = await callApi("https://api.example.com/upload", {
	method: "POST",
	body: toFormData(validatedData, { returnType: "inputType" }),
});
```

...
The `fetchApi` mode is designed for scenarios where you want most of the library's benefits (URL resolution, plugins, hooks) but don't want the response body parsed internally, just like the Fetch API.
...
Override the default object-to-string serialization:
...
Transform the request body into another Fetch-compatible body type:
...
await callApi("/api/upload", {
	method: "POST",
	body: { avatar: imageFile, name: "John" },

...
// @annotate: Using method prefixes with React Query for cleaner code
```
...
- **`resultMode: "onlySuccessWithException"`** - Returns just the data property with its exact type, perfect for React Query
...
// @annotate: Type of the data is automatically inferred from the schema
```
...
React Query's error handling works seamlessly with CallApi when `throwOnError: true` is set:

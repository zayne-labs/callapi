...
- `throwOnError`: true - This setting makes CallApi throw errors instead of returning them in the result object
- `resultMode`: "onlySuccessWithException" - This setting makes CallApi return just the data property with its exact type (i.e. `Todo[]` instead of `Todo[] | null`), since that's mostly all we care about in this scenario.

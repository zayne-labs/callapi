...
<Callout type="info">
	**Hooks vs Middleware:** Hooks observe events, so naturally their return values are ignored. Middleware
	on the other hand, controls execution flow and can modify or replace operations. Use hooks for side
	effects, middleware for control flow. See [Middleware](/docs/middlewares) for details.
</Callout>
...
const client = createFetchClient({
	onRequest: ({ request, options }) => {
		...
		// Add custom headers
		request.headers["X-Custom-ID"] = "123";
...
For a detailed walkthrough on using `refetch()` for token refreshing, see the [Authorization guide](/docs/authorization#refreshing-tokens).

<Callout type="warning">
	To prevent infinite loops, manual refetches are limited by the `refetchAttempts` option (defaults to
	`1`). Once the limit is reached, `refetch()` does nothing, and the request resolves with its error as usual.
</Callout>

Raise the limit for a client or a single request with `refetchAttempts`, or for one `refetch()` call with `maxAttempts`, which takes priority when set:

```ts title="api.ts"
const client = createFetchClient({
	// Applies to every request from this client
	refetchAttempts: 2,

	onSuccess: ({ options }) => {
		// Poll up to 10 times, overriding refetchAttempts for this call
		options.refetch({ maxAttempts: 10 });
	},
});
```
...
1. **The Plugin Level**: (covered in [`plugins`](/docs/plugins))
...
<Callout type="info">
	...
	[plugin](/docs/plugins) is often the better approach.
</Callout>
...
- The base client's `onResponse` is an **array**. The instance `onResponse` is **added** to this array. The combined array `[2.2 Base, 2.3 Base, 3.2 Instance]` is then executed.

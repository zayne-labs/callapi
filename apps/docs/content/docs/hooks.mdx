...
You can also customize the overall execution order (`mergedHooksExecutionOrder`) and whether hooks at the same level run sequentially or in parallel (`mergedHooksExecutionMode`) using options, but the default behavior is a common and predictable setup.

...
const CacheExtraOptionsSchema = z.object({
	cachePolicy: z.enum(["cache-first", "no-cache"]),
	cacheLifetimeMs: z.number().int().positive(),
});
...
const cachingPlugin = definePlugin(() => ({
	...
	middlewares: ({ options }: PluginSetupContext<z.infer<typeof CacheExtraOptionsSchema>>) => {
		...
		const cacheLifetime = options.cacheLifetimeMs ?? 60_000;
		...
		return {
			fetchMiddleware: (fetchImpl) => async (input, init) => {
				...
				if (cached && Date.now() - cached.timestamp < cacheLifetime) {
					...
					const cachedResponse = cached.data.clone();
...
const UploadExtraOptionsSchema = z.object({
	onUploadProgress: z
		.function()
...
const uploadProgressPlugin = definePlugin({
	...
	defineExtraOptions: () => UploadExtraOptionsSchema,

	middlewares: ({ options }: PluginSetupContext<z.infer<typeof UploadExtraOptionsSchema>>) => ({
		fetchMiddleware: (fetchImpl) => async (input, init) => {

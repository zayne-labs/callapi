...
Add plugins to individual requests when you need them for specific calls only:
...
**NOTE**: By default, passing `plugins` to a request replaces the base plugins. To keep base plugins and add new ones, use a callback:

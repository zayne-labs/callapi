...
Use this when the plugin's hooks or setup need extra fields. For plugins that only add request options or metadata, `definePlugin` is enough.
...
type AuthOptions = { apiKey: string; refreshToken?: string };
type AuthContext = GetCallApiContext<{ InferredExtraOptions: AuthOptions }>;

export const authPlugin = definePluginWithContext<AuthContext>()({
	id: "auth-plugin",

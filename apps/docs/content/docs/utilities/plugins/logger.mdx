...
The logger plugin provides detailed, structured logging for all HTTP request/response lifecycle events. Built on [consola](https://github.com/unjs/consola).
...
```package-install
@zayne-labs/callapi-plugins @zayne-labs/callapi consola @zayne-labs/toolkit-type-helpers
```

`@zayne-labs/callapi`, `consola` and `@zayne-labs/toolkit-type-helpers` are peer dependencies.
...
Toggle logging on/off. Can be a boolean for all logging, or an object for granular control over specific events.
`onResponse` enables both successful response and response error logs.
...
Provide a custom console-like object implementing this interface:
...
Use `redact` to remove sensitive values before verbose error data is logged:
...
const client = createFetchClient({
	plugins: [
		loggerPlugin({
			mode: "verbose",
			redact: (errorData) => redactSecrets(errorData),
		}),
	],
});
```

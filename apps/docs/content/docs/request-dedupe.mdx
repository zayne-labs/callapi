...
<Callout type="warn">
	The hash uses `JSON.stringify`, so non-JSON values such as `FormData`, `Blob` and `Headers` instances all hash
	to `{}`. Two uploads to the same URL with different `FormData` bodies get the same key and would cancel each
	other under the default `"cancel"` strategy. Set a custom `dedupeKey` or use `dedupeStrategy: "none"` for these
	requests.
</Callout>

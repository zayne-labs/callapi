...
```ts title="search.ts"
// Simulating a "search-as-you-type" feature.
// We use a fixed dedupeKey so logical "searches" cancel each other regardless of the query string.
...
// Calling these in rapid succession:
handleSearch("h");
handleSearch("he");
handleSearch("hel");
const { data } = await handleSearch("hello");
...
```ts title="api.ts"
// Parallel calls made at the same time will share a single underlying fetch.

...
The `createFetchClient` function accepts all standard [Fetch API options](https://developer.mozilla.org/en-US/docs/Web/API/fetch#parameters) alongside [CallApi's extended options](/docs/extra-options).

Options provided to `createFetchClient` act as base defaults. You can override these defaults at any time by passing the corresponding options directly to the `callApi` instance.
...
For a comprehensive deep-dive into validation strategies, including global validation, request-options (body, headers, params, query) validation, custom validators, and best practices, check out our [Validation Guide](/docs/validation).

...
This is useful when integrating with libraries that expect promises to reject on failure (like React Query, SWR, or TanStack Query).
...
This function will be called with the error context object, and you can throw an error based on the context.

For example, you can throw an error only if the response status is 401 (Unauthorized):

```ts twoslash title="api.ts"
import { callApi } from "@zayne-labs/callapi";

// Only throw for specific status codes
const { data } = await callApi("https://my-api.com/api/v1/session", {

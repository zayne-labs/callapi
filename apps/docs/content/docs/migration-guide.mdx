...
First, install CallApi alongside or instead of Ky:
...
First, install CallApi alongside or instead of Ofetch:
...
const { data } = await callApi("@post/upload", {
	...
	onRequestStream: ({ event }) => {
		...
		console.log(`Transferred: ${event.transferredBytes} / ${event.totalBytes} bytes`);
	},
});
```
...
- Update imports from `axios`/`ky`/`ofetch` to `@zayne-labs/callapi`
- Replace `axios.create()`, `ky.create()`, or `ofetch.create()` with `createFetchClient()`
- Update HTTP method calls to use `@method/path` syntax or the `method` option
- Convert interceptors to hooks (`onRequest`, `onSuccess`, `onError`, `onRequestError`, `onResponseError`)
- Update error handling to use result destructuring and `isHTTPError()` utility
- Replace `.json()` calls with automatic data extraction from the `data` property
- Update retry configuration (`retryAttempts`, `retryDelay` instead of `retry`)

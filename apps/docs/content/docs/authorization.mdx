...
Since the `auth` function is called right before every request, and manual `refetch()` preservation is built-in, you can handle `401 Unauthorized` errors easily:

...
const cachingPlugin = definePlugin({
  ...
  fetchInterceptor: (originalFetch) => async (input, init) => {
    ...
    const response = await originalFetch(input, init); // Calls base interceptor
...
const data = await client("/users", {
  ...
  fetchInterceptor: (originalFetch) => async (input, init) => {
    ...
    return originalFetch(input, newInit); // Calls plugin interceptor
  }
});
...
### Requirement 1: Fetch Interceptor as First-Class Feature

**User Story:** As a developer, I want to provide fetch interceptors at any configuration level (base config, plugins, or per-request), so that I can wrap the fetch function and control when and how network requests are made.
...
9. WHEN an interceptor calls the original fetch function THEN the request SHALL proceed to the next interceptor or the actual fetch implementation
10. WHEN an interceptor modifies the input or init before calling original fetch THEN those modifications SHALL be passed to the next layer

### Requirement 2: Options Access in Fetch Interceptor

**User Story:** As a developer, I want to access all configuration options within the fetch interceptor, so that I can make decisions based on the full request context.
...
### Requirement 3: Fetch Interceptor Composition Order

**User Story:** As a developer using interceptors at multiple levels, I want a predictable composition order, so that I can ensure interceptors interact correctly.
...
### Requirement 6: Error Handling in Fetch Interceptor

**User Story:** As a developer, I want errors in fetch interceptors to be handled gracefully, so that I get clear error messages.
...
**User Story:** As a developer, I want clear documentation and examples of fetch interceptors, so that I can use them effectively at any configuration level.
